...
public final class CameraManager
{
	...
	public CameraManager(Context context, PreviewCallback callback) {
	...
	public synchronized void openDriver(SurfaceView surfaceView) throws IOException {
		...
		theCamera.setPreviewDisplay(surfaceView.getHolder());
	...
	protected void setDisplayOrientation(Camera camera, int angle) {
		Method methodPolymorphic;
		try {
			methodPolymorphic = camera.getClass().getMethod("setDisplayOrientation", new Class[] { int.class });
			if (methodPolymorphic != null)
				methodPolymorphic.invoke(camera, new Object[] { angle });
		} catch (Exception e1) {
		}
	}
