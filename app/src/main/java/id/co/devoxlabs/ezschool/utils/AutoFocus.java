...
public final class AutoFocus implements Camera.AutoFocusCallback {
	...
	public AutoFocus(Context context, Camera camera) {
		this.camera = camera;
		SharedPreferences sharedPrefs = PreferenceManager.getDefaultSharedPreferences(context);
		String currentFocusMode = camera.getParameters().getFocusMode();
		useAutoFocus = sharedPrefs.getBoolean(Preference.KEY_AUTO_FOCUS, true) &&
		...
		if (useAutoFocus) {
			...
			if (!stopped && !focusing) {
				try {
					...
					Log.e(TAG, "Unexpected exception while focusing"+camera);
