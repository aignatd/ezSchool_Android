...
final class CameraConfigurationManager
{
	...
	void initFromCameraParameters(Camera camera,SurfaceView surfaceView) {
		...
		float frameH = SessionHelper.getInstance(context).getFrameHeight();
		float frameW = SessionHelper.getInstance(context).getFrameWidth();

		if(frameH != -1 && frameW != -1){
			theScreenResolution.set((int)frameW,(int)frameH);
		}else{
			Camera.Size bestSize = getBestPreviewSize(camera, surfaceView.getWidth(), surfaceView.getHeight());

			if(bestSize != null){
				//If we found the best display size
				theScreenResolution.set((int)frameW,(int)frameH);
				SessionHelper.getInstance(context).setFrameHeight(bestSize.height);
				SessionHelper.getInstance(context).setFrameWidth(bestSize.width);
			}
		}
	...
	void setDesiredCameraParameters(Camera camera, boolean safeMode) {
		...
		if (prefs.getBoolean(Preference.PrefAutoFocus, true)) {
			if (safeMode || prefs.getBoolean(Preference.PrefContFocus, true)) {
		...
		if (!safeMode) {
			if (prefs.getBoolean(Preference.PrefInvertScan, false)) {
			...
			if (!prefs.getBoolean(Preference.PrefDisableBarcodeSceneMode, true)) {
			...
			if (!prefs.getBoolean(Preference.PrefDisableMetering, true)) {
	...
	private void doSetTorch(Camera.Parameters parameters, boolean newSetting, boolean safeMode) {
		...
		if (!prefs.getBoolean(Preference.PrefDisableExposure, true)) {
			if (!safeMode) {
	...
	private Point findBestPreviewSizeValue(Camera.Parameters parameters, Point screenResolution) {
        ...
        Collections.sort(rawSupportedSizes, new SupportedCameraComparator());

		List<Camera.Size> filteredPreviewSizes = new ArrayList<>();
		for (Camera.Size filteredPreviewSize : rawSupportedSizes) {
			if(filteredPreviewSize.height <= 500){
				filteredPreviewSizes.add(filteredPreviewSize);
			}
		}

        if (Log.isLoggable(TAG, Log.INFO)) {
            StringBuilder filteredSizesString = new StringBuilder();
            for (Camera.Size filteredPreviewSize : rawSupportedSizes) {
                filteredSizesString.append(filteredPreviewSize.width).append('x')
                        .append(filteredPreviewSize.height).append(' ');
            }
            Log.i(TAG, "Raw preview sizes: " + filteredSizesString);
        }

		// Sort by size, descending
		List<Camera.Size> supportedPreviewSizes = new ArrayList<Camera.Size>(filteredPreviewSizes);
        ...
        Log.i(TAG, "filteredPreviewSizes.isEmpty() : " + filteredPreviewSizes.size());
		if (filteredPreviewSizes.size() > 0) {
			Camera.Size largestPreview = filteredPreviewSizes.get(0);
	...
	public class SupportedCameraComparator implements Comparator<Camera.Size> {
